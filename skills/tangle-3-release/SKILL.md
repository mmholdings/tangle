...
This skill runs after `tangle-2-implement` has converged (implementation done, testing gate green, Codex code review `APPROVED` or explicitly skipped). It is normally chained from tangle-2 in the same session, but can be invoked standalone in a fresh session.
...
### Standalone verification (fresh session, not chained from tangle-2)

If this skill was NOT chained from a tangle-2 session in the current conversation, verify before any release step:
...
Any failure blocks the release — fix or return to `tangle-2-implement` first.
...
(Week anchor: the Monday of the week Tangle Init was run. Python is used instead of `date -d` because macOS BSD `date` doesn't support it. If `python3` is not on PATH — e.g. Git Bash on Windows — use `python` instead.)
...
2. Content source:
   ...
   - **Skipped Codex**: write CR from `.claude/skills/tangle-review/cr-template.md` with body "Code review skipped — trivial change." Verdict: `APPROVED with observations`.
...
3. Run `bash .claude/skills/tangle-compact/count-tokens.sh docs/ARCHI.md` to check token count

**Warning: If ARCHI.md exceeds ~20,000 tokens**, warn the user:

> "ARCHI.md is at ~X tokens. Consider running `tangle-compact` to reduce it before committing."
...
Keep the pre-existing (non-Tangle) documentation aligned with the code:
