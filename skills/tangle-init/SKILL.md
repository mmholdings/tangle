---
name: tangle-init
description: Initialize Tangle workflow in a new project (creates docs structure and generates ARCHI.md)
disable-model-invocation: true
argument-hint: "name of the project to initialize"
---

# Tangle Initialization Mode

You are now in **initialization mode** for setting up the Tangle workflow.

## What is Tangle?

Tangle is a structured development workflow with four phases:
...
Initialize the Tangle workflow for the project: **$ARGUMENTS**
...
## Phase 6: Update Tangle Skills

After user validation, update the other Tangle skill files based on the **actual codebase architecture** documented in ARCHI.md.
...
1. **`tangle-1-plan`** - Technical considerations, guidance sections, documentation impact candidates
2. **`tangle-2-implement`** - Testing gate commands
3. **`tangle-3-release`** - Version file, week offset, tutorials
4. **`tangle-review`** - `checklist.md` and `cr-template.md` adapted to actual architecture
5. **`tangle-test`** - Test commands, structure, priorities
...
Replace the `[ADAPT_TO_PROJECT]` marker inside the plan template's **Documentation Impact** section with the project's actual living documentation — every non-Tangle document that code changes can leave stale, one bullet per doc with when it's affected. Draw from what Phase 2 discovered:
...
This list is what makes the tangle-3 Documentation Sync step effective — a doc missing here tends to drift silently.

Also resolve the `[ADAPT_TO_PROJECT]` marker in the **Prerequisites** list at the top of `tangle-1-plan`: if the project has living docs a plan must respect beyond ARCHI.md (e.g. an operations manual), list them with the condition under which they must be read; otherwise delete that line.
...
### 6.3 Update `tangle-2-implement` and `tangle-3-release`

The testing gate in `tangle-2-implement` and the standalone-verification block in `tangle-3-release` share the same command placeholders. The release ceremony customizations (version, week, tutorials) live in `tangle-3-release`:
...
The week Init is run becomes **Week 1** of the project. Capture the anchor date (Monday of the current week) and update the week formula in `tangle-3-release`.
...
- Remove the `[TUTORIAL_STEP]` block entirely from `tangle-3-release`
...
Then update the `[TUTORIAL_STEP]` block in `tangle-3-release` with the user's context:
...
Replace the `[LINT_COMMAND]`, `[TYPECHECK_COMMAND]`, and `[TEST_COMMAND]` placeholders in the tangle-2 Testing Gate AND the tangle-3-release standalone-verification block with the **actual commands** for this project (from ARCHI.md or discovered during exploration). For example:
...
| --------- | ---- | ------------------------------- |
| `X.Y.Z+1` | 1    | chore: initialize Tangle workflow |
```

- **Version**: SemVer format in backticks (e.g., `1.0.0`, `0.2.1`)
- **Week**: Project week number. Week 1 = the week when Tangle Init was run.
...
- **vX.Y.Z+1 (Tangle Initialization - Week 1, DD-MM-YYYY)**:
  - **Setup**: Initialized Tangle workflow with docs structure
...
- [ ] **Tangle skills updated** (Phase 6):
  - [ ] `[PROJECT_NAME]` placeholder replaced in all skills
  - [ ] `tangle-1-plan`: `[ADAPT_TO_PROJECT]` markers replaced with actual technical considerations
  - [ ] `tangle-1-plan`: Guidance sections replaced with project-specific patterns
  - [ ] `tangle-1-plan`: Documentation Impact candidates replaced with the project's actual living docs
  - [ ] `tangle-1-plan`: Prerequisites `[ADAPT_TO_PROJECT]` line resolved (project docs listed, or line deleted)
  - [ ] `tangle-1-plan`: Custom plan sections added (if user requested)
  - [ ] `tangle-2-implement`: Testing gate commands (`[LINT_COMMAND]`, `[TYPECHECK_COMMAND]`, `[TEST_COMMAND]`) replaced with actual commands
  - [ ] `tangle-3-release`: `[VERSION_FILE]` placeholder replaced
  - [ ] `tangle-3-release`: `[WEEK_ANCHOR_DATE]` placeholder replaced
  - [ ] `tangle-3-release`: `[MAIN_BRANCH]` placeholder replaced
  - [ ] `tangle-3-release`: Standalone-verification commands replaced with actual commands
  - [ ] `tangle-3-release`: Tutorial preference configured (if enabled: 5-tuto/ folder created + user context; if disabled: `[TUTORIAL_STEP]` block removed)
  - [ ] `tangle-review/checklist.md`: `[ADAPT_TO_PROJECT]` markers replaced with project-specific checklist sections
  - [ ] `tangle-review/cr-template.md`: Checklist section names updated to match adapted `checklist.md`
  - [ ] `tangle-test`: `[TEST_COMMAND_*]` placeholders replaced with actual commands
  - [ ] `tangle-test`: `[ADAPT_TO_PROJECT]` markers replaced with actual test structure/priorities
