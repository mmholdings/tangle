---
name: tangle-hotfix
description: Urgent fix bypassing full Tangle workflow
...
> **Warning**: Only use this for genuine emergencies. For regular bugs, use the full Tangle workflow (`tangle-1-plan` → `tangle-2-implement`).
...
- **Question**: "Is this a production-critical issue that cannot wait for the normal Tangle workflow?"
- **Options**: "Yes — critical issue" (security vulnerability, data corruption, service outage, or critical user-facing bug), "No — regular bug" (redirect to `tangle-1-plan` for proper workflow)

**If "No"**: Redirect to `tangle-1-plan` for proper workflow.
...
2. **Follow-up**: If deeper fixes are needed, create a proper Tangle plan
