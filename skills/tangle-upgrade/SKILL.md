---
name: tangle-upgrade
description: Upgrade Tangle workflow skills to a newer version while preserving project customizations
disable-model-invocation: true
argument-hint: "[path to new-Tangle folder]"
---

# Tangle Upgrade Mode

You are now in **upgrade mode** — merging a newer version of the Tangle workflow into this project's existing, customized Tangle skills.

## The Problem

Each project's Tangle skills have two interleaved layers:
...
The user must have copied the new generic Tangle skills into a staging folder before running this skill. Default location: `.claude/skills/new-Tangle/`

If `$ARGUMENTS` is provided, treat it as the path to the staging folder. Otherwise use `.claude/skills/new-Tangle/`.
...
Confirm the staging folder exists and contains Tangle skills:
...
> "No staging folder found at `<path>`. Copy the new Tangle workflow's `skills/` folder there first, then re-run."
...
**Pure workflow skills** (no project customizations): `tangle-compact`, `tangle-hotfix`, `tangle-research`, `tangle-init`, `codex-implement`, `codex-plan-review`, `codex-code-review`
...
**Customized skills** (have project-specific content): `tangle-1-plan`, `tangle-2-implement`, `tangle-3-release`, `tangle-review`, `tangle-test`

**Renamed in Tangle v2** — when the installed folder uses an old name, treat it as the same skill under its new name (merge into the new name, then delete the old folder):

| Installed (old) | Staging (new) |
|---|---|
| `tangle-3-review` | `tangle-review` |
| `tangle-4-test` | `tangle-test` |

`tangle-3-release` is **new in v2** but its project values (version file, week anchor, tutorial config) are **extracted from the old `tangle-2-implement`'s post-implementation steps** — categorize it as customized even though no folder exists yet.

**Other non-Tangle skills** in staging (e.g. future additions): Treat as new, or as pure workflow if they already exist.
...
Skill                 | Status              | Action
--------------------- | ------------------- | ------
tangle-1-plan           | Updated (customized) | Extract + merge
tangle-2-implement      | Updated (customized) | Extract + merge
tangle-3-release        | New (customized)     | New template + values from old tangle-2
tangle-review           | Renamed + updated    | Extract + merge, delete tangle-3-review/
tangle-test             | Renamed + updated    | Extract + merge, delete tangle-4-test/
tangle-compact          | Unchanged            | Skip
tangle-hotfix           | Unchanged            | Skip
tangle-init             | Updated (pure)       | Replace
tangle-research         | Unchanged            | Skip
...
**From tangle-2-implement/SKILL.md** (in v1 installs, the release values below live in its Post-Implementation steps; in v2 installs they live in `tangle-3-release/SKILL.md`):
- `PROJECT_NAME` — (confirm matches tangle-1-plan)
...
**From tangle-review/SKILL.md — or `tangle-3-review/` in v1 installs (checklist.md if already split):**
...
**From tangle-test/SKILL.md — or `tangle-4-test/` in v1 installs:**
...
### 3.1 Checklist Extraction (tangle-review)

**Old structure** (early v1): Checklist inline in `tangle-3-review/SKILL.md`
**New structure** (v2): Checklist in separate `tangle-review/checklist.md`, template in `tangle-review/cr-template.md` (late-v1 installs have these same files under `tangle-3-review/`)
...
### 3.2 Codex Integration (tangle-1-plan, tangle-2-implement)

**Old structure**: No Codex review steps
**New structure**: tangle-1-plan has Step 3 (Codex plan review), tangle-2-implement has Codex Code Review section

These are pure workflow additions — no project-specific content to migrate. They will be applied from the new template. The only project-specific part is the test commands in tangle-2-implement's Codex pre-step, which come from the extracted context.

### 3.3 Codex Skills (codex-plan-review, codex-code-review, codex-implement)

If not installed yet, these are entirely new — copy from staging directly. The review skills reference `tangle-review/checklist.md` and `tangle-review/cr-template.md`, which will be populated with project content. If already installed (late-v1), replace as pure workflow (see the `_common.sh` exception in Phase 1.2) — v1 prompt templates point at the old `tangle-3-review/` paths and must be replaced with the v2 versions.
...
#### tangle-2-implement/SKILL.md

1. Start from the new template (staging)
2. Replace `[PROJECT_NAME]` with extracted `PROJECT_NAME`
3. Replace `[LINT_COMMAND]`, `[TYPECHECK_COMMAND]`, `[TEST_COMMAND]` in the Testing Gate with extracted commands
   - If the old version didn't have Codex review (no test commands extracted), check the old tangle-4-test for test commands, or ask the user
4. Adapt the Integration impact check comment block to the project's integration/E2E tooling (from the old tangle-4-test content if present)

#### tangle-3-release/SKILL.md (new in v2 — values come from the old tangle-2)
...
#### tangle-review/SKILL.md + checklist.md + cr-template.md (was `tangle-3-review` in v1)
...
#### tangle-test/SKILL.md (was `tangle-4-test` in v1)
...
```bash
grep -rn '\[ADAPT_TO_PROJECT\|\[PROJECT_NAME\]\|\[VERSION_FILE\]\|\[WEEK_ANCHOR_DATE\]\|\[TEST_COMMAND\]\|\[LINT_COMMAND\]\|\[TYPECHECK_COMMAND\]\|\[TUTORIAL_STEP\]\|\[MAIN_BRANCH\]' .claude/skills/tangle-*/
```
...
- `codex-code-review/prompts/start.tpl` and `resume.tpl` reference `.claude/skills/tangle-review/checklist.md` — confirm it exists, and that no template still points at the old `tangle-3-review/` path
- `codex-code-review/prompts/synthesize.tpl` and `codex-code-review/SKILL.md` reference `.claude/skills/tangle-review/cr-template.md` — confirm it exists
- `tangle-1-plan` references `codex-plan-review/scripts/start.sh` and `resume.sh`; `tangle-2-implement` and `tangle-research` invoke each skill's own wrapper scripts — confirm `codex-implement`, `codex-code-review`, and `codex-ask` each ship `scripts/{start,resume,reset,show}.sh` (they pin `STATE_DIR` and delegate to `codex-plan-review`), and that `codex-plan-review/scripts/key.sh` exists
...
2. Report completion:
   > "Tangle workflow upgraded. The staging folder has been removed. You can `git diff .claude/skills/` to review all changes before committing."
...
This is the most common upgrade path. The Codex skills are "New" — copy directly. The Codex integration in tangle-1-plan and tangle-2-implement comes from the new template and needs no project-specific content except test commands.
...
If the old version predates the Codex review pre-step and tangle-4-test doesn't have extractable commands, ask the user:
